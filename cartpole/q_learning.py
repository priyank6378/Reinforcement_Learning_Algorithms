...
class QLearningAgent:
    ...
    '''
    def __init__(self, model, target_model, env, exp_replay, step_size=0.5,  gamma=0.9,  batch_size=90, frequency=10, tau=1, epsilon=0.05):
        self.model = model
    ...
    def train_batch(self, sample_exps):
        ...
        y_batch =  r_batch + self.gamma * np.max(next_state_q_vals, axis=1) * (1 - done_batch)
        y_batch = q_vals +  self.step_size * (y_batch - q_vals)
        
        with GradientTape() as tape:
            q_vals = self.model(s1_batch, training=True)
            q_vals = tf.gather(q_vals, a_batch, axis=1, batch_dims=1)
            loss = tf.reduce_mean(tf.square(y_batch - q_vals))
